...
def test_eqF():
    ...
    pf = np.array([4, 1, 5])
    Y = np.array([6, 3, 0.4])
    expected_F_vals = np.array(
        [[0.225, 0.6, 4.4], [1.2, 2.4, 10.56], [0.24, 0.24, 1.408]]
    )
    expected_F = pd.DataFrame(expected_F_vals, index=index)
    test_F = household.eqF(beta, py, Y, pf)
    print("Type = ", type(test_F))
    print(beta)
    assert_frame_equal(expected_F, test_F, check_dtype=False)
...
def test_eqXp():
    ...
    print(alpha)
    assert_frame_equal(expected_Xp, test_Xp, check_dtype=False)
